...
namespace guideXOS.GUI {
    internal class GXMScriptWindow : Window {
        ...
        public override void OnInput(){ base.OnInput(); if(!Visible||IsMinimized||IsTombstoned) return; int mx=Control.MousePosition.X; int my=Control.MousePosition.Y; bool left=Control.MouseButtons.HasFlag(MouseButtons.Left);
            if(left){ if(!_clickLatch){ // buttons
                    ...
                    // dropdowns
                    for(int i=0;i<_dropdowns.Count;i++){ var d=_dropdowns[i]; int rx=X+d.X; int ry=Y+d.Y; if(mx>=rx&&mx<=rx+d.W&&my>=ry&&my<=ry+d.H){ d.Open=!d.Open; _clickLatch=true; continue; }
                        if(d.Open){ int itemY=ry+d.H; for(int it=0; it<d.Items.Count; it++){ int ih=WindowManager.font.FontSize+6; int iy=itemY+it*ih; if(mx>=rx&&mx<=rx+d.W&&my>=iy&&my<=iy+ih){ d.Selected=it; d.Open=false; _clickLatch=true; RunActions(2,d.Id,d.Items[it]); break; } }
                        }
                    }
                    // lists
                    for(int i=0;i<_lists.Count;i++){ var l=_lists[i]; int rx=X+l.X; int ry=Y+l.Y; if(mx>=rx&&mx<=rx+l.W&&my>=ry&&my<=ry+l.H){ int rowH=WindowManager.font.FontSize+6; int rel=my-ry; int idx=rel/rowH; if(idx>=0 && idx<l.Items.Count){ l.Selected=idx; _clickLatch=true; RunActions(2,l.Id,l.Items[idx]); } }
                    }
                }
            } else { _clickLatch=false; }
        }
        private void RunActions(int type,int id,string value){ for(int i=0;i<_callbacks.Count;i++){ var cb=_callbacks[i]; if(cb.Type==type && cb.Id==id){ string act = cb.Action ?? string.Empty; string arg = cb.Arg ?? string.Empty; if(value!=null){ // replace $VALUE token
        ...
        private void ExecuteAction(string action,string arg){ // supported: MSG, OPENAPP, CLOSE
            ...
            char[] ca = new char[a.Length]; for(int i=0;i<a.Length;i++){ char c=a[i]; if(c>='a'&&c<='z') c=(char)(c-32); ca[i]=c; } a=new string(ca);
            if(a=="MSG") { Notify(arg); }
            else if(a=="OPENAPP") { if(Desktop.Apps!=null && arg!=null) { Desktop.Apps.Load(arg); } }
            else if(a=="CLOSE") { this.Visible=false; }
        }
        ...
        public override void OnDraw(){ base.OnDraw(); if(IsMinimized) return; // labels
            ...
            // dropdowns
            for(int i=0;i<_dropdowns.Count;i++){ var d=_dropdowns[i]; int rx=X+d.X; int ry=Y+d.Y; Framebuffer.Graphics.FillRectangle(rx,ry,d.W,d.H,0xFF2E2E2E); string txt=d.Selected>=0 && d.Selected<d.Items.Count? d.Items[d.Selected]:"(select)"; WindowManager.font.DrawString(rx+6,ry+(d.H/2-WindowManager.font.FontSize/2),txt,d.W-12,WindowManager.font.FontSize); // arrow
